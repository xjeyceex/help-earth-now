...
const mapToCandidate = (data: any): Candidate => {
  return {
    ...data,
    party: data.party.includes('Republican')
      ? Party.Republican
      : data.party.includes('Democratic')
      ? Party.Democratic
...
const getCandidatesForState = (state: string, candidates: Candidate[]) => {
  const governors = candidates.filter(
    (candidate) => candidate.position === 'gov'
  );
  const senators = candidates.filter(
    (candidate) => candidate.position === 'sen'
  );
  ...
  const stateSpecificCandidates = (groupItems: Candidate[]) => {
    const stateSpecific = groupItems.filter(
      (candidate) => candidate.state.toLowerCase() === state.toLowerCase()
    );
    const allCandidates = groupItems.filter(
      (candidate) => candidate.state === 'All'
    );
    ...
    const filteredCandidates = allCandidates.filter(
      (allCandidate) =>
        !stateSpecific.some(
          (stateCandidate) =>
            stateCandidate.name === allCandidate.name &&
            stateCandidate.position === allCandidate.position
        )
    );
  ...
  return [
    {
      ...
      items: stateSpecificCandidates([
        ...
        },
        {
          ...
          rating: '30% from LCV.',
          description:
            "In 2019, Trump's delay of a crucial $19.1 billion disaster relief bill for North Carolina postponed vital flood mitigation projects, leading to significant impacts from subsequent storms, including the flooding caused by Hurricane Helene in 2023.",
    ...
    const fetchData = async () => {
      ...
      if (location && location.state) {
        ...
        const stateAbbreviation = Object.keys(routeToStateMap).find(
          (key) =>
            routeToStateMap[key].toLowerCase() === fullStateName.toLowerCase()
        );

        if (stateAbbreviation) {
          const candidates = await fetchCandidateData(stateAbbreviation);
          const stateCandidates = getCandidatesForState(
            stateAbbreviation,
            candidates
          );
        ...
        {filteredCandidates.length > 0 ? (
          filteredCandidates.map((group, groupIndex) => (
            <React.Fragment key={group.group}>
              <div
                className={`flex items-center justify-center text-center who-help-${
                  groupIndex + 1
                } p-4 md:p-8 border border-gray-300`}
                ...
                return (
                  <div
                    ...
                    className={`col-span-2 p-4 md:p-8 border border-gray-300 ${
                      candidate.party === Party.Democratic
                        ? 'bg-blue-600 text-white'
                        : candidate.party === Party.Republican
